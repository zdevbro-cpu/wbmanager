...
const FILING_RULES = [
  ...
  { parent: 'asset', fileType: '검사증', code: 'DOC-03-01-005' },
  { parent: 'asset', fileType: '제원표', code: 'DOC-03-01-006' },
  // 계약서는 자산이 아니라 계약서끼리 모인다. 문서가 자산에도 연결되므로 자산 문서함에서는 그대로 보인다.
  { parent: 'asset', fileType: '계약서', code: 'DOC-05-01-005' },
  // 종류를 가리지 못한 자산 파일은 미분류 대신 「자산 > 차량」에 둔다.
  // 갈 곳이 없어 미분류로 가면 차량번호로 찾을 수 없다(리뷰회의 5-9).
  { parent: 'asset', fileType: null, code: 'DOC-03-01' },
